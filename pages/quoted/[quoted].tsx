...
export const getServerSideProps = async (context: any) => {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/tweets/${quoted}`
  );
      ...
      <div className="QuotedContainer">
        ...
        <div className="centerGrid">
          <div className="quotedHeader">
            <Link href="/">
              <p>{<BsArrowLeft cursor="pointer" />}</p>
            </Link>
            <h2>Quoted Tweets </h2>
          </div>
          <div>
            {quotedProps.quoted?.length == 0 ? (
              <h2 className="noQuotesText"> Nothing to see here yet </h2>
            ) : (
              quotedProps.quoted?.map((quoted: any) => (
