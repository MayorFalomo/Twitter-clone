...
const ProfilePage = (props: any) => {
  ...
  const [following, setNoOfFollowing] = useState<any>(
    ...
    <ProfileStyled>
      <div className={props.editProfileModal ? "opaque" : "profilePageStyled"}>
        <div className="subProfileStyle">
          ...
          <div
            className={
              props.editProfileModal ? "editProfileModal" : "removeModal"
            }
          >
          ...
          <div className="userDetailsContainer">
            ...
            <p style={{ color: "#575B5F", fontWeight: 500 }}>
              {currentUser?.usersAt}{" "}
            </p>
            <div
              className="usersBio"
            ...
            >
              <div className="usersExtraInfo">
                <span style={{ color: "#575B5F" }}>
                  {<CiLocationOn />} {currentUser?.location}{" "}
                </span>
                <span style={{ color: "#575B5F" }} className="usersLink">
                  {" "}
                  {<AiOutlineLink />} {currentUser?.links}{" "}
                </span>
                <span style={{ color: "#575B5F", fontWeight: 600 }}>
                  {<BsBalloon />} Born {currentUser?.birthday}{" "}
                </span>
                <br />
              </div>
              <p style={{ color: "#575B5F", fontWeight: 600 }}>
                {<BiCalendar />} Joined{" "}
                {moment(currentUser?.createdAt).format("MMMM YYYY")}{" "}
              </p>
            </div>
            <div className="followContainer" style={{ marginBottom: 70 }}>
              <Link href={"/following/" + currentUser?.username}>
                <span>{currentUser?.following?.length} Following </span>
              </Link>
              <Link href={"/followers/" + currentUser.username}>
