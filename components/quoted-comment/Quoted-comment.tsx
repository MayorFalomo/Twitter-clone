...
const Quoted = (props: any) => {
  ...
  const handleQuotedTweet = async (e: any) => {
    ...
    await axios
      .put(
        `${process.env.NEXT_PUBLIC_BASE_URL}/tweets/quote-tweet/`,
        quotedData
      )
      .catch((err) => console.log(err));
    setComments(" ");
    ...
    setSingleTweets({
      ...singleTweets,
      quoted: [...singleTweets.quoted, quotedData],
    });
  };
  const handleClose = () => {
    ...
    <QuotedCommentStyle>
      <div className="commentModalContainer">
        <div className="commentModalClose">
          {<MdClose onClick={handleClose} fontSize={30} cursor="pointer" />}{" "}
        </div>
            ...
            <div className="tweetDetailsContainer">
              <div style={{ padding: 10 }}>
                <div className="tweetDetails">
                  <span
                    style={{
                      backgroundImage: `url(${singleTweets?.profileDp})`,
                    }}
                  ...
                  <span style={{ fontWeight: 400 }}>
                    {singleTweets?.username}{" "}
                  </span>
                  <span
                    style={{ fontWeight: 400, color: "rgb(113,118,123)" }}
                    className="usersAt"
                  >
                    {singleTweets?.usersAt}
                  </span>
                  <span
                    style={{ fontWeight: 400, color: "rgb(113,118,123)" }}
                    className="createdAt"
                  >
                    {moment(singleTweets?.createdAt).format("MMMM D")}{" "}
                  </span>
                </div>
                <p className="tweet">{singleTweets?.tweet} </p>
                <Link
                  href={"/posts/" + singleTweets?._id}
                  style={{ color: "#1d9aef", opacity: 0.4 }}
                >
                  Show this Thread{" "}
                </Link>
              </div>
              {singleTweets?.picture?.length > 1 ? (
                <div
                  style={{ backgroundImage: `url(${singleTweets?.picture})` }}
                  className="pictures"
                >
                  {" "}
                </div>
              ...
              {singleTweets?.video?.length > 0 && (
                <video src={singleTweets?.video} controls className="videos" />
              )}
            </div>
